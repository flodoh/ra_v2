...
class textanalyzer(object):
    ...
    def analyzeText(self, text=''):
        ...
        newDaleChallMissingWords = self.countNewDaleChallWordsNotInList(text)
        newDaleChallWords = wordCount - newDaleChallMissingWords
...
#        print ' Number of words not Dale-Chall list: ' + str(newDaleChallMissingWords)
    ...
    def getWords(self, text=''):
        ...
        for word in words:
            if word in self.special_chars or word == " ":
                ...
                #added
                new_word = new_word.replace("(","").replace(")","").replace("-","").replace(";","").replace(":","").replace(" ","").replace("\n","")
                if not new_word == "":
...
#            self.lang = NaiveBayes().classifyText(" " .join(words))
    ...
    #author: Marcus Voß
    def countNewDaleChallWordsNotInList(self, text=''):
        words = self.getWords(text)
        newDaleChallMissingWordCount = 0

        NewDaleChallWordsFile = open('../nltk_contrib/dale_chall_wordlist.txt').read()
       
        NewDaleChallWordsList = NewDaleChallWordsFile.split(';')
        for word in words:
            if word.lower() not in NewDaleChallWordsList:
                if not word.isdigit():
                    newDaleChallMissingWordCount += 1
        return newDaleChallMissingWordCount
    
    def countNewDaleChallWordsNotInList_enhanced(self, text=''):
        words = self.getWords(text)
        newDaleChallMissingWordCount = 0

        NewDaleChallWordsFile = open('../nltk_contrib/dale_chall_wordlist.txt').read()
        privacy_file = open('../nltk_contrib/privacy_wordlist.txt').read()
       
        NewDaleChallWordsList = NewDaleChallWordsFile.split(';')
        privacy_list = privacy_file.split(';')
        
        for word in words:
            if word.lower() not in NewDaleChallWordsList and word.lower() not in privacy_list:
                if not word.isdigit():
                    newDaleChallMissingWordCount += 1
        return newDaleChallMissingWordCount
...
# changed due to error in classifyer (found no tickle file)
        ta = textanalyzer("eng")
        ta.analyzeText(text)
        pass
